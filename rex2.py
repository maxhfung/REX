...
style = 'background: rgb(43, 71, 79);color: rgb(140, 195, 212); font-family:bahnschrift;font-weight:400;font-size: 20px;'
funte = "bahnschrift"
blue = "#27c9d9"
...
fnt = resource_path("fonts/bahnschrift_3.ttf")
...
#       From settings, get the GPIB and ASRL names, addresses, and cmds.
    ...
    def initUI(self):
        ...
        self.label_slogan = QLabel("Calibration <span style='font-weight:400;'>Assistant</span>")
        self.label_slogan.setStyleSheet("QLabel {font-family: bahnschrift; font-weight:200; font-size:25px;font-weight:lighter;color:#63a5b8;text-align:left;}")

        link_sheet = ''' QPushButton{font-family: bahnschrift; font-weight:200; font-size:16px;border: 0px;color:rgb(113, 188, 209);padding: 0px 0px 0px 0px; padding-right:10px;}
                              QPushButton:hover{color: #2eecff;} QPushButton:focus {border: 0px;outline: none;background-color:transparent;selection-background-color:transparent;}'''
        link_tb_sheet = ''' QToolButton{font-family: bahnschrift; font-weight:200; font-size:16px;border: 0px;color:rgb(113, 188, 209);padding: 0px 0px 0px 0px; padding-right:10px;}
                              QToolButton:hover{color: #2eecff;}QToolButton:focus {border: none;outline: none;}'''
        ...
        self.body_frame.setStyleSheet("background-color:rgb(10, 20, 23);padding: 0px 0px 0px 0px;")
        ...
        self.PlotCanvas.move(self.Console.width-50,96)
        ...
        self.MonitorFrame.setFixedSize(self.Monitor.width,self.Monitor.height)
        self.MonitorFrame.setContentsMargins(0,0,0,0)
        self.monitor_layout = QHBoxLayout(self.MonitorFrame)
        self.monitor_layout.addWidget(self.Monitor)
        self.monitor_layout.setContentsMargins(0,0,0,0)
        self.MonitorFrame.move(self.Console.width,self.height-self.Monitor.height)
        ...
        self.plot_label_legend_1.setStyleSheet("background-color:transparent;padding: 0px 0px 0px 0px;color:'Aqua';font-size:12px;")
        self.plot_label_legend_2 = QLabel("",self)
        self.plot_label_legend_2.setFixedWidth(300)
        self.plot_label_legend_2.move(410,215)
        self.plot_label_legend_2.setStyleSheet("background-color:transparent;padding: 0px 0px 0px 0px;color:'MediumSpringGreen';font-size:12px;")
        ...
        self.lineEdit_pressure.returnPressed.connect(self.Console.next_event)
        self.lineEdit_pressure.returnPressed.connect(self.console_next)
    ...
    def console_next(self):
        ...
        z=[0,0.127556574,0.133207613,0.100742637,0.055462793,-0.078206777,-0.081492641,-0.015511857,-0.016562688,-0.00677017,-0.0002693763,-0.074917687]
        ...
        if 2 <= self.Console.step_no <= 6:
            self.PlotCanvas.progressBar(self.Console.step_no,int(self.Console.setup_full_scale))
    ...
    def gui_move_psi_label(self,useless=''):

        if len(self.lineEdit_pressure.text()) > 0:
            _str = self.lineEdit_pressure.text()
        else:
            _str = self.lineEdit_pressure.placeholderText()
        _shift = 45 * len(_str)
...
class Console(QWidget):
    ...
    def __init__(self, parent = MainWindow):
        ...
        self.float_validator = QDoubleValidator(0.,25.,2,self)
    ...
    def initUI(self):
        ...
        self.lineEdit_barometer.setValidator(self.float_validator)
            ...
            if self.comboBox_psi_type.currentIndex() != 0:
                ...
                if self.lineEdit_barometer.isEnabled():
                    ...
                    if len(self.lineEdit_barometer.text()) > 0:
                        if float(self.lineEdit_barometer.text()) < 14.7*3:
                            valid_count += 1
                        else:
...
class PlotCanvas(FigureCanvas):
    ...
    def __init__(self, parent=MainWindow, width=5, height=4, dpi=100):
        ...
        self.cmap = matplotlib.colors.ListedColormap(["OrangeRed","RebeccaPurple","MediumSpringGreen","RebeccaPurple","OrangeRed"])
        var = 0.8325
    ...
    def plot(self, _init = True, x=[0,400,800,1200,1600,2000,2000,1600,1200,800,400,0],y=[2.4254,8.5509,14.6393,20.7161,26.789,32.835,32.834,26.7674,20.6804,14.5967,8.5129,2.4026], z=[0,-0.5,-0.499207613,0.49900742637,0.50,-0.378206777,-0.281492641,-0.115511857,0.116562688,0.20677017,0.302693763,0.474917687],span=2000):
        ...
        self.ax.set_facecolor((0.03922,  0.07843,  0.09020))
...
#        self.ax.yaxis.grid(color=(0.16863,  0.27843,  0.30980))
    ...
    def updateData(self, ticks=True, x=[0,400,800,1200,1600,2000,2000,1600,1200,800,400,0],y=[2.4254,8.5509,14.6393,20.7161,26.789,32.835,32.834,26.7674,20.6804,14.5967,8.5129,2.4026],z=[0,0.127556574,0.133207613,0.100742637,0.055462793,-0.078206777,-0.081492641,-0.015511857,-0.016562688,-0.00677017,0.002693763,-0.074917687], span=2000):
        ...
        if len(x) > 0:
            self.ax_3.plot([-100,span + 100],[0,0], color='MediumSpringGreen',alpha=0.5)
        ...
        self.ax.plot(x,y,marker='|', markersize=15, color='Aqua')
        ...
        if 7 > len(x) >= 1:
            ...
            self.ax.fill_between([ line_index_x, x[int(len(x)-1)] ], [ 0 , y[len(y)-1] ], alpha=0.01, color="DeepSkyBlue")
        if 12 >= len(x) >= 7:
            self.ax_4.set_ylim(bottom=0,top=max(y)+max(y)*0.05)
            self.ax.fill_between([x[len(x)-1],span],[y[len(x)-1], max(y)],alpha=0.1)
            self.ax.fill_between([ line_index_x, x[int(len(x)-1)] ], [ 0 , y[len(y)-1] ], alpha=0.02, color="DeepSkyBlue")
            if 12 > len(x):
                self.ax.fill_between(x,y,y2=linspace(0 ,y[len(y)-1], len(y)), alpha=0.01, color="DeepSkyBlue")
    ...
    def progressBar(self,step,span):
        colors = ["DeepSkyBlue","Aqua","SpringGreen","MediumSeaGreen","DeepSkyBlue","Aqua","SpringGreen","MediumSeaGreen"]
        self.ax_3.clear()

        for spine in ['bottom','left','right','top']:
            self.ax_3.spines[spine].set_visible(False)
        self.ax_3.set_xticklabels([])
        self.ax_3.set_yticklabels([])
        self.ax_3.xaxis.set_ticks_position('none')
        self.ax_3.yaxis.set_ticks_position('none')

        self.ax_3.fill_between([0,1], [(step-2)*0.25,(step-2)*0.25],[(step-3)*0.25,(step-3)*0.25], alpha=0.2, color=colors[step-3])
        self.ax_3.set_ylim(0,1)
        self.ax_3.set_xlim(0,1)
        if step == 4:
            self.ax_3.fill_between([0,1], [(step-3)*0.25,(step-3)*0.25],[(step-4)*0.25,(step-4)*0.25], alpha=0.2, color=colors[step-4])
        if step == 5:
            self.ax_3.fill_between([0,1], [(step-3)*0.25,(step-3)*0.25],[(step-4)*0.25,(step-4)*0.25], alpha=0.2, color=colors[step-4])
            self.ax_3.fill_between([0,1], [(step-4)*0.25,(step-4)*0.25],[(step-5)*0.25,(step-5)*0.25], alpha=0.2, color=colors[step-5])
        if step == 6:
            self.ax_3.fill_between([0,1], [(step-3)*0.25,(step-3)*0.25],[(step-4)*0.25,(step-4)*0.25], alpha=0.2, color=colors[step-4])
            self.ax_3.fill_between([0,1], [(step-4)*0.25,(step-4)*0.25],[(step-5)*0.25,(step-5)*0.25], alpha=0.2, color=colors[step-5])
            self.ax_3.fill_between([0,1], [(step-5)*0.25,(step-5)*0.25],[(step-6)*0.25,(step-6)*0.25], alpha=0.2, color=colors[step-6])
...
class Monitor(QGraphicsView):
    ...
    def __init__(self, parent = MainWindow):
        ...
        self.width, self.height = 570, 110
        self.ui_style_sheet = "QGraphicsView {background-color: #000;}"
        self.setStyleSheet(self.ui_style_sheet)
        self.initUI()
    ...
    def initUI(self):
        ...
        self.monitorScene_1= QGraphicsScene()
        self.setSceneRect(0,0,self.width-10,self.height-10)
        self.fitInView(0,0,self.width,self.height,Qt.KeepAspectRatio)
        self.setStyleSheet('Background-color:rgb(16, 27, 31);')

        self.multi_title = QGraphicsTextItem()
        self.multi_font = QFont()
        self.multi_font.setFamily("Bahnschrift")
        self.multi_font.setPointSize(12)
        self.multi_title.setDefaultTextColor(QColor(140, 195, 212))
        self.multi_title.setFont(self.multi_font)
        self.multi_title.setTextWidth(self.width-52)
        self.multi_title.setPos(40+30,2)
        self.multi_title.setHtml("<span style='color:#3c6675'>MULTIMETER</span>    <span style='color:MediumSpringGreen'>ON</span>")
        # self.multi_title.setHtml("<span style='color:#3c6675'>MULTIMETER</span>    <span style='color:#49869c'>SEARCHING...</span>")
        # self.multi_title.setHtml("<span style='color:#3c6675'>MULTIMETER</span>    <span style='color:#223c45'>NOT FOUND</span>")

        self.gauge_title = QGraphicsTextItem()
        self.gauge_font = QFont()
        self.gauge_font.setFamily("Bahnschrift")
        self.gauge_font.setPointSize(12)
        self.gauge_title.setDefaultTextColor(QColor(140, 195, 212))
        self.gauge_title.setFont(self.gauge_font)
        self.gauge_title.setTextWidth(self.width-52)
        self.gauge_title.setPos(self.width/2+30,2)
        self.gauge_title.setHtml("<span style='color:#3c6675'>GAUGE</span>    <span style='color:MediumSpringGreen'>ON</span>")
        # self.gauge_title.setHtml("<span style='color:#3c6675'>GAUGE</span>    <span style='color:#49869c'>SEARCHING...</span>")
        # self.gauge_title.setHtml("<span style='color:#3c6675'>GAUGE</span>    <span style='color:#223c45'>NOT FOUND</span>")

        self.multi_name = QGraphicsTextItem()
        self.multi_name.setDefaultTextColor(QColor(140, 195, 212))
        self.multi_name.setFont(self.multi_font)
        self.multi_name.setTextWidth(self.width-52)
        self.multi_name.setPos(40+30,80)
        self.multi_name.setHtml("<span style='color:#3c6675'>HP3478A</span> <span style='color:MediumTurquoise'>1</span>")
        # self.multi_title.setHtml("<span style='color:#3c6675'>MULTIMETER</span>    <span style='color:#49869c'>SEARCHING...</span>")
        # self.multi_title.setHtml("<span style='color:#3c6675'>MULTIMETER</span>    <span style='color:#223c45'>NOT FOUND</span>")

        self.gauge_name = QGraphicsTextItem()
        self.gauge_name.setDefaultTextColor(QColor(140, 195, 212))
        self.gauge_name.setFont(self.gauge_font)
        self.gauge_name.setTextWidth(self.width-52)
        self.gauge_name.setPos(self.width/2+30,80)
        self.gauge_name.setHtml("<span style='color:#3c6675'>MENSOR 2500</span> <span style='color:MediumTurquoise'>A</span>")
        # self.gauge_title.setHtml("<span style='color:#3c6675'>GAUGE</span>    <span style='color:#49869c'>SEARCHING...</span>")
        # self.gauge_title.setHtml("<span style='color:#3c6675'>GAUGE</span>    <span style='color:#223c45'>NOT FOUND</span>")

        self.monitor_voltage = QGraphicsTextItem()
        self.monitor_font_v = QFont()
        self.monitor_font_v.setFamily("Bahnschrift")
        self.monitor_font_v.setPointSize(70)
        self.monitor_font_v.setWeight(700)
        self.monitor_voltage.setDefaultTextColor(QColor(105, 137, 148))
        self.monitor_voltage.setFont(self.monitor_font_v)
        self.monitor_voltage.setTextWidth(self.width-52)
        self.monitor_voltage.setPos(40+30,15)
        self.monitor_voltage.setHtml('32.00'+"<span style='font-size:25px;font-weight:200'>mV</span>")

        self.monitor_pressure = QGraphicsTextItem()
        self.monitor_font_p = QFont()
        self.monitor_font_p.setFamily("Bahnschrift")
        self.monitor_font_p.setPointSize(70)
        self.monitor_font_p.setWeight(700)
        self.monitor_pressure.setDefaultTextColor(QColor(105, 137, 148))
        self.monitor_pressure.setFont(self.monitor_font_p)
        self.monitor_pressure.setTextWidth(self.width-52)
        self.monitor_pressure.setPos(self.width/2+30,15)
        self.monitor_pressure.setHtml('1000'+"<span style='font-size:25px;font-weight:200'>PSI</span>")

        self.monitorScene_1.addItem(self.multi_title)
        self.monitorScene_1.addItem(self.gauge_title)
        self.monitorScene_1.addItem(self.monitor_voltage)
        self.monitorScene_1.addItem(self.monitor_pressure)
        self.monitorScene_1.addItem(self.multi_name)
        self.monitorScene_1.addItem(self.gauge_name)

        self.setScene(self.monitorScene_1)
        self.setFixedSize(self.width,self.height)
        self.setContentsMargins(0,0,0,0)
...
if __name__ == '__main__':
     ...
     family = QFontDatabase.applicationFontFamilies(db)
     view = MainWindow()
     view.show()
